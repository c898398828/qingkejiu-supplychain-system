...
import { clearHuzhuAuth, getHuzhuToken } from '@/lib/huzhuAuth'
...
api.interceptors.request.use((config) => {
  const token = getHuzhuToken() || localStorage.getItem('token')
  if (token) {
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (status === 401 && currentPath !== '/login' && !isLoginRequest) {
      clearHuzhuAuth()
...
export interface HuzhuApiResponse<T> {
  code: number
  message: string
  data: T
}

export interface HuzhuPageResult<T> {
  records: T[]
  total: number
  pageNum: number
  pageSize: number
...
export interface HuzhuDashboardOverview {
  baseCount: number
...
export interface HuzhuProductionAnalysis {
  totalTasks: number
...
export interface HuzhuSalesAnalysis {
  orderCount: number
  salesAmount: number
...
export interface HuzhuSupplyChainAnalysis {
  purchaseCount: number
  purchaseAmount: number
  inventoryCount: number
  inventoryValue: number
...
export interface HuzhuMonthlyTrend {
  labels: string[]
  purchaseAmountTrend: number[]
  salesAmountTrend: number[]
  productionQtyTrend: number[]
  orderCountTrend: number[]
  qualityPassRateTrend: number[]
}

export const huzhuAuthService = {
  async login(username: string, password: string): Promise<HuzhuApiResponse<{ token: string; user: any }>> {
    const response = await api.post('/auth/login', { username, password })
    return response.data
  },
}

export const huzhuDashboardService = {
  async getOverview(): Promise<HuzhuApiResponse<HuzhuDashboardOverview>> {
    const response = await api.get('/dashboard/overview')
    return response.data
  },
}

export const huzhuAnalysisService = {
  async getProduction(): Promise<HuzhuApiResponse<HuzhuProductionAnalysis>> {
    const response = await api.get('/analysis/production')
    return response.data
  },
  async getSales(): Promise<HuzhuApiResponse<HuzhuSalesAnalysis>> {
    const response = await api.get('/analysis/sales')
    return response.data
  },
  async getSupplyChain(): Promise<HuzhuApiResponse<HuzhuSupplyChainAnalysis>> {
    const response = await api.get('/analysis/supply-chain')
    return response.data
  },
  async getMonthlyTrend(months = 12): Promise<HuzhuApiResponse<HuzhuMonthlyTrend>> {
    const response = await api.get('/analysis/monthly-trend', { params: { months } })
    return response.data
  },
}

export const huzhuCrudService = {
  async page<T>(endpoint: string, params: Record<string, any>): Promise<HuzhuApiResponse<HuzhuPageResult<T>>> {
    const response = await api.get(`${endpoint}/page`, { params })
    return response.data
  },

  async create(endpoint: string, payload: Record<string, any>): Promise<HuzhuApiResponse<boolean>> {
    const response = await api.post(endpoint, payload)
    return response.data
  },

  async update(endpoint: string, id: number, payload: Record<string, any>): Promise<HuzhuApiResponse<boolean>> {
    const response = await api.put(`${endpoint}/${id}`, payload)
    return response.data
  },

  async remove(endpoint: string, id: number): Promise<HuzhuApiResponse<boolean>> {
